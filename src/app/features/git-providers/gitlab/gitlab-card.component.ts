...
import { GitlabService, SCOPES } from './gitlab.service';
...
@Component({
  ...
  <div class="flex items-center justify-between mb-4">
    <div class="flex items-center space-x-3">
      ...
      <h2 class="text-xl font-semibold dark:text-white">GitLab</h2>
    </div>
    @if (connected()) {
  ...
  @if (!connected()) {
    <div>
      ...
      <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">
        <span i18n="@@GET_TOKEN_HERE">Get your token here:</span> Settings > Access Tokens > Add new token
      </p>
    </div>
  }
...
export class GitlabCardComponent {
  ...
  protected onSubmit(): void {
    ...
    this.gitlabService.validateAndStoreToken(token).subscribe({
      next: () => {
        this.form.reset();
  ...
  protected disconnectProvider(): void {
    ...
    this.gitlabService.disconnectProvider();
