...
import { GithubService, SCOPES } from './github.service';
...
@Component({
  ...
  <div class="flex items-center justify-between mb-4">
    <div class="flex items-center space-x-3">
      ...
      <h2 class="text-xl font-semibold dark:text-white">Github</h2>
    </div>
    @if (connected()) {
  ...
  @if (!connected()) {
    <div>
      ...
      <!-- https://github.com/settings/tokens -->
        <span i18n="@@GET_TOKEN_HERE">Get your token here:</span> Settings > Developer settings > Personal access tokens > Tokens (classic) > Generate new token
      </p>
    </div>
  }

  <form [formGroup]="form" (ngSubmit)="onSubmit()" class="space-y-4">
    @if (!connected() || showTokenInput()) {
...
export class GithubCardComponent {
  ...
  protected onSubmit(): void {
    ...
    this.githubService.validateAndStoreToken(token).subscribe({
      next: () => {
        this.form.reset();
  ...
  protected disconnectProvider(): void {
    ...
    this.githubService.disconnectProvider();
