...
import { BitbucketService, SCOPES } from './bitbucket.service';
...
@Component({
  ...
  <div class="flex items-center justify-between mb-4">
    <div class="flex items-center space-x-3">
      ...
      <h2 class="text-xl font-semibold dark:text-white">Bitbucket</h2>
    </div>
    @if (connected()) {
  ...
  @if (!connected()) {
    <div>
      ...
      <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">
        <span i18n="@@GET_TOKEN_HERE">Get your token here:</span> Settings > Access tokens > Create token
      </p>
    </div>
  }
...
export class BitbucketCardComponent {
  ...
  protected onSubmit(): void {
    ...
    this.bitbucketService.validateAndStoreToken(token).subscribe({
      next: () => {
        this.form.reset();
  ...
  protected disconnectProvider(): void {
    ...
    this.bitbucketService.disconnectProvider();
