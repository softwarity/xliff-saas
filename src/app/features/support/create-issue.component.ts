      ...
      <div class="bg-white dark:bg-dark-800 rounded-lg shadow-md p-6">
        <form [formGroup]="ticketForm" autocomplete="off">
          ...
          <div class="flex justify-end">
            <button type="button" routerLink="/support" class="button stroke-primary mr-4" i18n="@@CANCEL_BUTTON">Cancel</button>
            <button type="button" (click)="onSubmit('Bug')" [disabled]="ticketForm.invalid || isSubmitting()" class="button flat-warning mr-4 min-w-[140px] flex items-center justify-center">
              @if (isSubmitting()) {
              ...
              <span i18n="@@SUBMIT_BUG_BUTTON"i18n>Submit Bug</span>
            </button>
            <button type="button" (click)="onSubmit('Feature')" [disabled]="ticketForm.invalid || isSubmitting()" class="button flat-primary min-w-[140px] flex items-center justify-center">
              @if (isSubmitting()) {
                <svg class="animate-spin mr-2 h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                  <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                  <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
              ...
              <span i18n="@@SUBMIT_FEATURE_BUTTON">Submit Feature</span>
            </button>
          </div>
        </form>
      </div>
