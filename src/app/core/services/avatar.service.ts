...
export class AvatarService {
  ...
  constructor() {
    this.auth.user$.pipe(
      filter(user => !!user),
      mergeMap(user => this.supabase.from('user_metadata').select('avatarUrl').eq('user_id', user.id).single()),
      ...
      mergeMap(user => {
        ...
        return from(this.supabase.storage.from('avatars').upload(fileName, file, { upsert: true })).pipe(
          mergeMap(({ error: uploadError, data }: { error: StorageError | null, data: { id: string; path: string; fullPath: string } | null}) => {
            ...
            return from(this.supabase.from('user_metadata').update({ avatarUrl: data!.path }).eq('user_id', user.id)).pipe(
